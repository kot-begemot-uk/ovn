...
static void
build_lswitch_input_port_sec(struct ovn_port *op,
                void *arg
)
{
    ...
    ovn_lflow_add_with_hint(li->lflows, op->od, S_SWITCH_IN_PORT_SEC_L2, 50,
                            ds_cstr(&match), ds_cstr(&actions),
                            &op->nbsp->header_);

    if (op->nbsp->n_port_security) {
        build_port_security_ip(P_IN, op, li->lflows, &op->nbsp->header_);
        build_port_security_nd(op, li->lflows, &op->nbsp->header_);
    }
}
...
)
{
    ...
    if (lsp_is_enabled(op->nbsp)) {
        ...
        ovn_lflow_add_with_hint(li->lflows, op->od, S_SWITCH_OUT_PORT_SEC_L2,
    ...
    if (op->nbsp->n_port_security) {
        build_port_security_ip(P_OUT, op, li->lflows, &op->nbsp->header_);
    }
    ds_destroy(&match);
    ds_destroy(&actions);
}
...
)
{
   ...
   /* Egress tables 8: Egress port security - IP (priority 0)
     ...
     *                 (priority 100). */
    ovn_lflow_add(li->lflows, od, S_SWITCH_OUT_PORT_SEC_IP, 0, "1", "next;");
    ovn_lflow_add(li->lflows, od, S_SWITCH_OUT_PORT_SEC_L2, 100, "eth.mcast",
                  "output;");
...
)
{
     ...
     * goto next. (priority 0) moved from build_lswitch_port_sec
     */
   
    ovn_lflow_add(li->lflows, od, S_SWITCH_IN_PORT_SEC_ND, 0, "1", "next;");
    ovn_lflow_add(li->lflows, od, S_SWITCH_IN_PORT_SEC_IP, 0, "1", "next;");
                    ...
                    void *arg)
{
    ...
    if ((!strcmp(op->nbsp->type, "localnet")) ||
        ...
        ds_clear(&match);
        ds_put_format(&match, "inport == %s", op->json_key);
        ovn_lflow_add_with_hint(li->lflows, op->od, S_SWITCH_IN_ARP_ND_RSP,
    ...
    if (!strcmp(op->nbsp->type, "virtual")) {
        ...
        for (vparent = strtok_r(tokstr, ",", &save_ptr); vparent != NULL;
            ...
            struct ovn_port *vp = ovn_port_find(li->ports, vparent);
            if (!vp || vp->od != op->od) {
...
static void
build_lswitch_per_port_dest_unicast (
                    struct ovn_port *op,
                    void *arg)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
     ...
     */
    if (!strcmp(op->nbsp->type, "router")) {
        build_lswitch_rport_arp_req_flows(op->peer, op->od, op, li->lflows,
                                          &op->nbsp->header_);
    }
    ...
    for (size_t i = 0; i < op->nbsp->n_addresses; i++) {
        ...
        if (ovs_scan(op->nbsp->addresses[i],
            ...
            ovn_lflow_add_with_hint(li->lflows, op->od, S_SWITCH_IN_L2_LKUP,
                                    50, ds_cstr(&match),
                                    ds_cstr(&actions),
                                    &op->nbsp->header_);
        } else if (!strcmp(op->nbsp->addresses[i], "unknown")) {
            if (lsp_is_enabled(op->nbsp)) {
                ovn_multicast_add(li->mcgroups, &mc_unknown, op);
...
static void *ovn_port_thread(void *arg) {
 
    struct worker_control *control = (struct worker_control *) arg;
    struct ovn_port_pool *workload;
    struct lswitch_build_info *li;  
    struct ovn_port *op;
    int bnum;
    ...
    while (!seize_fire()) {
        sem_wait(&control->fire);
        workload = (struct ovn_port_pool *) control->workload;
        li = (struct lswitch_build_info *) control->data;
        if (li->ports != NULL) {
            for (bnum = control->id; 
                    bnum <= li->ports->mask;
...
static void
build_lrouter_logical_ingress_table_1_od(struct ovn_datapath *od,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
    ...
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_LOOKUP_NEIGHBOR, 100,
                  "arp.op == 2",
                  REGBIT_LOOKUP_NEIGHBOR_RESULT" = "
                  "lookup_arp(inport, arp.spa, arp.sha); next;");

    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_LOOKUP_NEIGHBOR, 100, "nd_na",
                  REGBIT_LOOKUP_NEIGHBOR_RESULT" = "
                  "lookup_nd(inport, nd.target, nd.tll); next;");

    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_LOOKUP_NEIGHBOR, 100, "nd_ns",
                  REGBIT_LOOKUP_NEIGHBOR_RESULT" = "
                  "lookup_nd(inport, ip6.src, nd.sll); next;");

    /* For other packet types, we can skip neighbor learning.
     * So set REGBIT_SKIP_LOOKUP_NEIGHBOR to 1. */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_LOOKUP_NEIGHBOR, 0, "1",
                  REGBIT_SKIP_LOOKUP_NEIGHBOR" = 1; next;");

    /* Flows for LEARN_NEIGHBOR. */
    /* Skip Neighbor learning if not required. */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_LEARN_NEIGHBOR, 100,
                  REGBIT_SKIP_LOOKUP_NEIGHBOR" == 1 || "
                  REGBIT_LOOKUP_NEIGHBOR_RESULT" == 1", "next;");

    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_LEARN_NEIGHBOR, 90,
...
static void
build_lrouter_logical_ingress_table_1_op(struct ovn_port *op,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
...
static void
build_lrouter_logical_ingress_table_3_od(struct ovn_datapath *od,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
     ...
     * (priority 100). */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 100,
                  "ip4.src_mcast ||"
     ...
     * IPs are handled with priority-90 flows.
     */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 85,
                  "arp || nd", "drop;");

    /* Allow IPv6 multicast traffic that's supposed to reach the
     * router pipeline (e.g., router solicitations).
     */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 84, "nd_rs || nd_ra",
                  "next;");

    /* Drop other reserved multicast. */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 83,
                  "ip6.mcast_rsvd", "drop;");

    /* Allow other multicast if relay enabled (priority 82). */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 82,
                  "ip4.mcast || ip6.mcast",
                  od->mcast_info.rtr.relay ? "next;" : "drop;");

    /* Drop Ethernet local broadcast.  By definition this traffic should
     * not be forwarded.*/
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 50,
                  "eth.bcast", "drop;");

    /* TTL discard */
    ds_clear(&match);
    ds_put_cstr(&match, "ip4 && ip.ttl == {0, 1}");
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 30,
                  ds_cstr(&match), "drop;");

    /* Pass other traffic not already handled to the next table for
     * routing. */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_IP_INPUT, 0, "1", "next;");
...
static void
build_lrouter_logical_ingress_table_3_op(struct ovn_port *op,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
    ...
    if (op->lrp_networks.n_ipv4_addrs) {
        ...
        ovn_lflow_add_with_hint(li->lflows, op->od, S_ROUTER_IN_IP_INPUT, 100,
                                ds_cstr(&match), "drop;",
                                &op->nbrp->header_);
        ...
        ds_put_format(&actions,
            ...
            "flags.loopback = 1; "
            "next; ");
        ovn_lflow_add_with_hint(li->lflows, op->od, S_ROUTER_IN_IP_INPUT, 90,
                                ds_cstr(&match), ds_cstr(&actions),
                                &op->nbrp->header_);
    }
    ...
    for (int i = 0; i < op->lrp_networks.n_ipv4_addrs; i++) {
        ...
        ovn_lflow_add_with_hint(li->lflows, op->od, S_ROUTER_IN_IP_INPUT, 40,
                                ds_cstr(&match), ds_cstr(&actions),
                                &op->nbrp->header_);
    }
    ...
    SSET_FOR_EACH (ip_address, &all_ips_v4) {
        ...
        ovn_lflow_add(li->lflows, op->od, S_ROUTER_IN_IP_INPUT, 90,
                      ds_cstr(&match), ds_cstr(&actions));
    }
    ...
    if (!smap_get(&op->od->nbr->options, "chassis")
        ...
        for (int i = 0; i < op->lrp_networks.n_ipv4_addrs; i++) {
            ...
            const char *action = "icmp4 {"
                                 ...
                                 "icmp4.code = 3; "
                                 "next; };";
            ovn_lflow_add_with_hint(li->lflows, op->od, S_ROUTER_IN_IP_INPUT,
    ...
    if (has_drop_ips) {
        ...
        ovn_lflow_add_with_hint(li->lflows, op->od, S_ROUTER_IN_IP_INPUT, 60,
                                ds_cstr(&match), "drop;",
                                &op->nbrp->header_);
    }
...
static void
build_lrouter_dhcp6_reply(struct ovn_port *op,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
    ...
    for (size_t i = 0; i < lrp_networks.n_ipv6_addrs; i++) {
        ...
        ovn_lflow_add(li->lflows, op->od, S_ROUTER_IN_IP_INPUT, 100,
                      ds_cstr(&match), ds_cstr(&actions));
    }
...
static void
build_lrouter_ip_input(struct ovn_port *op,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
...
)
{
    ...
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_DEFRAG, 0, "1", "next;");
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_UNSNAT, 0, "1", "next;");
    ovn_lflow_add(li->lflows, od, S_ROUTER_OUT_SNAT, 0, "1", "next;");
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_DNAT, 0, "1", "next;");
    ovn_lflow_add(li->lflows, od, S_ROUTER_OUT_UNDNAT, 0, "1", "next;");
    ovn_lflow_add(li->lflows, od, S_ROUTER_OUT_EGR_LOOP, 0, "1", "next;");

    /* Send the IPv6 NS packets to next table. When ovn-controller
     * generates IPv6 NS (for the action - nd_ns{}), the injected
     * packet would go through conntrack - which is not required. */
    ovn_lflow_add(li->lflows, od, S_ROUTER_OUT_SNAT, 120, "nd_ns", "next;");
    ...
    for (int i = 0; i < od->nbr->n_nat; i++) {
        ...
        if (!strcmp(nat->type, "snat")
            ...
            if (!od->l3dgw_port) {
                ...
                ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_UNSNAT,
                                        90, ds_cstr(&match),
                                        ds_cstr(&actions),
                                        &nat->header_);
            } else {
        ...
        if (!strcmp(nat->type, "dnat")
            ...
            if (!od->l3dgw_port) {
                ...
                ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_DNAT, 100,
                                        ds_cstr(&match), ds_cstr(&actions),
                                        &nat->header_);
            } else {
                /* Distributed router. */
        ...
        if (od->l3dgw_port) {
            if (!strcmp(nat->type, "snat")) {
                ...
                ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_IP_INPUT,
            ...
            if (!sset_contains(&nat_entries, nat->external_ip)) {
                ...
                ovn_lflow_add_with_hint(li->lflows, od,
        ...
        if (od->l3dgw_port && (!strcmp(nat->type, "dnat")
            ...
            ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_OUT_UNDNAT, 100,
                                    ds_cstr(&match), ds_cstr(&actions),
                                    &nat->header_);
        }
        ...
        if (!strcmp(nat->type, "snat")
            ...
            if (!od->l3dgw_port) {
                 ...
                 * priority. */
                ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_OUT_SNAT,
        ...
        if (distributed) {
            ...
            ds_put_format(&match,
                          ...
                          od->l3dgw_port->json_key,
                          nat->logical_port);
            ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_ADMISSION, 50,
                                    ds_cstr(&match), "next;",
                                    &nat->header_);
        }
        ...
        if (distributed) {
            ...
            ds_put_format(&match, "ip%s.src == %s && outport == %s",
                          ...
                          nat->logical_ip,
                          od->l3dgw_port->json_key);
            ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_GW_REDIRECT,
        ...
        if (od->l3dgw_port) {
            ...
            ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_OUT_EGR_LOOP, 100,
                                    ds_cstr(&match), ds_cstr(&actions),
                                    &nat->header_);
        }
    }
    ...
    if (dnat_force_snat_ip && !od->l3dgw_port) {
        ...
        ds_put_format(&match, "ip && ip%s.dst == %s",
                      ...
                      dnat_force_snat_ip);
        ovn_lflow_add(li->lflows, od, S_ROUTER_IN_UNSNAT, 110,
        ...
        ovn_lflow_add(li->lflows, od, S_ROUTER_OUT_SNAT, 100,
                      ds_cstr(&match), ds_cstr(&actions));
    }
    if (lb_force_snat_ip && !od->l3dgw_port) {
        ...
        ds_put_format(&match, "ip && ip%s.dst == %s",
                      ...
                      lb_force_snat_ip);
        ovn_lflow_add(li->lflows, od, S_ROUTER_IN_UNSNAT, 100,
                      ds_cstr(&match), "ct_snat;");
    ...
    if (!od->l3dgw_port) {
        ...
        * we can do it here, saving a future re-circulation. */
        ovn_lflow_add(li->lflows, od, S_ROUTER_IN_DNAT, 50,
    ...
    for (int i = 0; i < od->nbr->n_load_balancer; i++) {
        ...
        struct ovn_lb *lb =
            ovn_lb_find(li->lbs, &nb_lb->header_.uuid);
        ovs_assert(lb);
        ...
        for (size_t j = 0; j < lb->n_vips; j++) {
            ...
            if (!sset_contains(&all_ips, lb_vip->vip)) {
                ...
                ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_DEFRAG,
            ...
            add_router_lb_flow(li->lflows, od, &match, &actions, prio,
                               lb_force_snat_ip, lb_vip, proto,
                               nb_lb, li->meter_groups, &nat_entries);
        }
    }
...
static void
build_lrouter_packet_size(struct ovn_datapath *od,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
    ...
    /* Packets are allowed by default. */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_CHK_PKT_LEN, 0, "1",
                  "next;");
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_LARGER_PKTS, 0, "1",
                  "next;");
    ...
    if (od->l3dgw_port && od->l3redirect_port) {
        ...
        ds_put_format(&actions,
                      ...
                      " next;", gw_mtu);
        ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_CHK_PKT_LEN, 50,
                                ds_cstr(&match), ds_cstr(&actions),
                                &od->l3dgw_port->nbrp->header_);

        for (size_t i = 0; i < od->nbr->n_ports; i++) {
            struct ovn_port *rp = ovn_port_find(li->ports,
                                                od->nbr->ports[i]->name);
            ...
            ds_put_format(&actions,
                ...
                gw_mtu - 18);
            ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_LARGER_PKTS,
                                    50, ds_cstr(&match), ds_cstr(&actions),
                                    &rp->nbrp->header_);
        }
    }
...
static void
build_lrouter_gw_redirect(struct ovn_datapath *od,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
    ...
    if (od->l3dgw_port && od->l3redirect_port) {
        ...
        ds_put_format(&actions, "outport = %s; next;",
                      od->l3redirect_port->json_key);
        ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_GW_REDIRECT, 50,
                                ds_cstr(&match), ds_cstr(&actions),
                                stage_hint);
    }

    /* Packets are allowed by default. */
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_GW_REDIRECT, 0, "1", "next;");
...
static void
build_lrouter_arp_request_od(struct ovn_datapath *od,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
    ...
    for (int i = 0; i < od->nbr->n_static_routes; i++) {
        ...
        ovn_lflow_add_with_hint(li->lflows, od, S_ROUTER_IN_ARP_REQUEST, 200,
                                ds_cstr(&match), ds_cstr(&actions),
                                &route->header_);
    }

    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_ARP_REQUEST, 100,
    ...
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_ARP_REQUEST, 100,
                  ...
                  "output; "
                  "};");
    ovn_lflow_add(li->lflows, od, S_ROUTER_IN_ARP_REQUEST, 0, "1", "output;");
...
static void
build_lrouter_egress_delivery(struct ovn_port *op,
                void *arg
)
{
    struct lswitch_build_info *li = (struct lswitch_build_info *) arg;
    ...
    if (op->od->mcast_info.rtr.relay) {
        ...
        ds_put_format(&actions, "eth.src = %s; output;",
                      op->lrp_networks.ea_s);
        ovn_lflow_add(li->lflows, op->od, S_ROUTER_OUT_DELIVERY, 110,
                      ds_cstr(&match), ds_cstr(&actions));
    }

    ds_clear(&match);
    ds_put_format(&match, "outport == %s", op->json_key);
    ovn_lflow_add(li->lflows, op->od, S_ROUTER_OUT_DELIVERY, 100,
...
build_lrouter_stage_2_od(struct ovn_datapath *od,
    ...
    build_lrouter_packet_size(od, arg);
    build_lrouter_gw_redirect(od, arg);
    build_lrouter_arp_request_od(od, arg);
}
...
build_lrouter_stage_2_op(struct ovn_port *op,
    ...
    build_lrouter_logical_ingress_table_1_op(op, arg);
    ...
    build_lrouter_egress_delivery(op, arg);
}
