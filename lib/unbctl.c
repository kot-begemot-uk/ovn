...
VLOG_DEFINE_THIS_MODULE(unbctl);

COVERAGE_DEFINE(unbctl_received);
COVERAGE_DEFINE(unbctl_replied);

struct unbctl_command {
    const char *usage;
                      ...
                      const char *argv[] OVS_UNUSED, void *aux OVS_UNUSED)
{
    ...
    for (i = 0; i < shash_count(&commands); i++) {
        ...
        const struct unbctl_command *command = node->data;

        ds_put_format(&ds, "  %-23s %s\n", node->name, command->usage);
    }
    free(nodes);

    unbctl_command_reply(conn, ds_cstr(&ds));
    ds_destroy(&ds);
}

static void
unbctl_version(struct unbctl_conn *conn, int argc OVS_UNUSED,
                const char *argv[] OVS_UNUSED, void *aux OVS_UNUSED)
{
    unbctl_command_reply(conn, ovs_get_program_version());
}

/* Registers a unbctl command with the given 'name'.  'usage' describes the
 ...
 * 'cb' should reply by calling unbctl_command_reply() or
 * unbctl_command_reply_error() before it returns, but if the command cannot
...
void
unbctl_command_register(const char *name, const char *usage,
                         int min_args, int max_args,
                         unbctl_cb_func *cb, void *aux)
{
    struct unbctl_command *command;
    struct unbctl_command *lookup = shash_find_data(&commands, name);
...
static void
unbctl_command_reply__(struct unbctl_conn *conn,
                        bool success, const char *body)
{
    struct json *body_json;
    struct jsonrpc_msg *reply;

    COVERAGE_INC(unbctl_replied);
...
/* Replies to the active unbctl connection 'conn'.  'result' is sent to the
 * client indicating the command was processed successfully.  Only one call to
 * unbctl_command_reply() or unbctl_command_reply_error() may be made per
 * request. */
void
unbctl_command_reply(struct unbctl_conn *conn, const char *result)
{
    unbctl_command_reply__(conn, true, result);
}

/* Replies to the active unbctl connection 'conn'. 'error' is sent to the
 * client indicating an error occurred processing the command.  Only one call to
 * unbctl_command_reply() or unbctl_command_reply_error() may be made per
 * request. */
void
unbctl_command_reply_error(struct unbctl_conn *conn, const char *error)
{
    unbctl_command_reply__(conn, false, error);
}

/* Creates a unbctl server listening on 'path', which for POSIX may be:
 *
 ...
 * sets '*serverp' to the new unbctl_server (or to NULL if 'path' was "none"),
 * otherwise to NULL. */
int
unbctl_server_create(const char *path, struct unbctl_server **serverp)
{
    *serverp = NULL;
    ...
    unbctl_command_register("list-commands", "", 0, 0, unbctl_list_commands,
                             NULL);
    unbctl_command_register("version", "", 0, 0, unbctl_version, NULL);

    struct unbctl_server *server = xmalloc(sizeof *server);
...
static void
process_command(struct unbctl_conn *conn, struct jsonrpc_msg *request)
{
    char *error = NULL;
...
static int
run_connection(struct unbctl_conn *conn)
{
    int error, i;
...
void
unbctl_server_run(struct unbctl_server *server)
{
    if (!server) {
        return;
    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (!error) {
            struct unbctl_conn *conn = xzalloc(sizeof *conn);
...
void
unbctl_server_wait(struct unbctl_server *server)
{
...
void
unbctl_server_destroy(struct unbctl_server *server)
{
    if (server) {
        struct unbctl_conn *conn, *next;
...
const char *
unbctl_server_get_path(const struct unbctl_server *server)
{
    return server ? server->path : NULL;
}

/* On POSIX based systems, connects to a unbctl server socket.  'path' should
 * be the name of a unbctl server socket.  If it does not start with '/', it
...
int
unbctl_client_create(const char *path, struct jsonrpc **client)
{
    struct stream *stream;
    int error;
 ...
 * '*err' if not NULL. */
int
unbctl_client_transact(struct jsonrpc *client, const char *command, int argc,
